...
import type { CategoryBadge, PostCategory } from '@/entities/post/model/types';

export const BoardPage = () => {
  const [currentCategory, setCurrentCategory] = useState<PostCategory>('all');
  const categories: CategoryBadge[] = ['event', 'activity', 'partnership', 'release', 'others'];

  const dummyPosts = Array.from({ length: 10 }).map((_, i) => ({
    id: i,
    title: `🌟 2025년 제${i + 1}차 부트캠프 일정 안내`,
    content: `Planit Bootcamp에서 진행되는 ${i + 1}번째 세션에 대한 안내입니다. 자세한 일정과 신청 방법은 공지사항에서 확인해주세요.`,
