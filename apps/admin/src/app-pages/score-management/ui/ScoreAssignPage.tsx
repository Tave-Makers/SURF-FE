...
export const ScoreAssignPage = () => {
  ...
  const { data: categories = [], isLoading, isError } = useActivityTypesQuery();
            ...
            <ul className="pb-7">
              {category.criteria.map((criterion) => (
                <li key={criterion.id}>
                  <button
                    type="button"
                    className="text-body-body6 text-foreground-normal w-full px-13 py-10 text-left"
                    onClick={() =>
                      router.push(PAGE_ROUTES.SCORE_MNG_ASSIGN_TARGET(encodeURIComponent(criterion.id)))
                    }
                  >
                    {criterion.label}
                  </button>
