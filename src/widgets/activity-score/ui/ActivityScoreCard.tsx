...
export default function ActivityScoreCard({ score, records, mode }: ActivityScoreCardProps) {
  return (
    <section aria-labelledby="activity-score-title" className="flex flex-col gap-[2.5rem]">
      {/* 점수 표시 */}
      <h2 className="flex h-[3.375rem] items-center justify-center gap-5">
      ...
      {/* 활동 아이콘 + 툴팁 */}
      {mode === 'REWARD' && (
        <div className="flex justify-center gap-16" role="list" aria-label="상점 활동 목록">
          {/** 상점: 단일 활동 */}
      ...
      {mode === 'PENALTY' && (
        <div className="flex justify-center gap-18" role="list" aria-label="벌점 활동 목록">
          {/** 벌점: 지각/결석 그룹 */}
