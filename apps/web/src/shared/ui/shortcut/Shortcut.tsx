...
export const Shortcut = ({ type, label, imageSrc, onClick }: ShortcutProps) => {
  if (type === 'circle') {
    return (
      <button type="button" className="flex w-full flex-col items-center gap-7" onClick={onClick}>
        <div className="bg-background-normal-lighter h-[2.5rem] w-[2.5rem] overflow-hidden rounded-full">
  ...
  return (
    <button
      type="button"
      className="bg-background-normal-lighter rounded-5 border-border-secondary shadow-lifted relative flex aspect-161/101 w-full overflow-hidden border"
      onClick={onClick}
    >
